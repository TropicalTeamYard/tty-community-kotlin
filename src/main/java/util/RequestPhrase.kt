...
class RequestPhrase (private val req: HttpServletRequest){
    ...
    fun getBlogFiles(blogId: String, author: String): HashMap<String, File> {
        if (list.isNullOrEmpty()) {
            return HashMap()
        }
        try {
            for (item in list!!) {
                if (!item.isFormField) {
                    val file = File(author+ "_" + item.name)
//                    val outFile = File(conf?.getString("root")+"/${conf?.getString("blog_pics")}", file.name)
